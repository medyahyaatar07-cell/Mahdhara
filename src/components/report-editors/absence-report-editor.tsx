...
import { AbsenceReportPreview } from "@/components/report-preview";
import { saveAbsenceReportAction } from "@/app/(app)/reports/actions";
import type { AbsenceReportRow } from "@/lib/types";

export interface AbsenceReportInitial {
  reportTitle: string;
  madrasaName: string;
...
export function AbsenceReportEditor({
  reportDate,
  initial,
  defaults,
  hasExistingReport,
  ...
  reportDate: string;
  initial: AbsenceReportInitial;
  defaults: AbsenceReportInitial;
  ...
  const saveOnly = useMemo(
    () => saveAbsenceReportAction.bind(null, reportDate, false),
    [reportDate]
  );
  const saveAndPdf = useMemo(
    () => saveAbsenceReportAction.bind(null, reportDate, true),
    [reportDate]
  );

  function set<K extends keyof AbsenceReportInitial>(key: K, val: AbsenceReportInitial[K]) {
    setValues((v) => ({ ...v, [key]: val }));
  }

  function setRow(index: number, patch: Partial<AbsenceReportRow>) {
    setValues((v) => ({
      ...v,
      ...
      <Card className="space-y-4 p-4 sm:p-6">
        {hasExistingReport && (
          <p className="rounded-xl bg-amber-100 px-3 py-2 text-sm font-bold text-amber-800 dark:bg-amber-900/30 dark:text-amber-300">
            يوجد تقرير غياب محفوظ مسبقًا بهذا التاريخ. سيتم تحديثه عند الحفظ.
          </p>
        )}
        ...
        <div>
          <p className="mb-2 text-sm font-bold text-foreground/80">
            الطلاب الغائبون ({values.rows.length})
          </p>
          <div className="space-y-2">
            {values.rows.length === 0 && (
              <p className="rounded-xl border-2 border-dashed border-border p-4 text-center text-sm text-foreground/50">
                لا يوجد طلاب مسجَّلون كغائبين في هذا اليوم.
              </p>
            )}
